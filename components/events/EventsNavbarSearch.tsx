...
function EventsNavbarSearchInner({ isMobile }: { isMobile?: boolean }) {
...
export default function EventsNavbarSearch({ isMobile }: { isMobile?: boolean }) {
  return (
    <Suspense fallback={
      isMobile ? (
        <div className="flex-1 relative">
          <div className="w-full h-8 bg-white/5 border border-white/10 rounded-lg animate-pulse" />
        </div>
      ) : (
        <div className="relative w-48 sm:w-64 mr-2 h-9 bg-[rgba(255,255,255,0.05)] border border-white/10 rounded-full animate-pulse" />
      )
    }>
      <EventsNavbarSearchInner isMobile={isMobile} />
    </Suspense>
  );
}
