...
export default function OrganizationCard({ organization }: { organization: OrganizationDirectoryEntry }) {
  ...
  return (
    <article className="wac-card group overflow-hidden p-5 flex flex-col sm:flex-row items-start sm:items-stretch gap-5 relative hover:-translate-y-0.5 transition-transform min-h-[140px]">
      ...
      <div className="absolute top-0 left-0 w-[40%] h-[150%] bg-gradient-to-r from-emerald-500/[0.04] to-transparent pointer-events-none -translate-y-1/4" />
      ...
      <div className="shrink-0 pt-0.5 relative z-10 w-16">
        {organization.logo_url ? (
          ...
          >
            <div className="absolute inset-[-4px] rounded-full border border-emerald-500/30 opacity-60" />
            <img
              src={organization.logo_url}
              alt={displayName}
              className="w-16 h-16 rounded-full border border-emerald-500/20 object-cover relative z-10"
            />
          </button>
        ) : (
          <div className="w-16 h-16 rounded-full border border-emerald-500/20 bg-emerald-500/10 flex items-center justify-center text-lg font-semibold uppercase text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.15)] relative">
            <div className="absolute inset-[-4px] rounded-full border border-emerald-500/30 opacity-60" />
            {initials ? initials : <Landmark size={22} className="text-emerald-400" strokeWidth={1.5} />}
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col flex-1 min-w-0 self-stretch pr-2 relative z-10">
        ...
        {/* Entity Badge */}
        <span className="text-[9px] font-bold uppercase tracking-widest text-emerald-400/80 block mb-1">
          Organization
        </span>

        <Link href={profileHref} className="group/title w-fit max-w-full mb-0.5">
          <div className="flex flex-wrap items-center gap-1.5">
            <h2 className="text-lg font-bold text-white/95 leading-tight group-hover/title:text-emerald-400 transition-colors truncate">
              {displayName}
            </h2>
            {organization.is_verified && <VerifiedBadge size="md" className="shrink-0 -mt-0.5" />}
          </div>
        </Link>
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-auto">
          ...
          {organization.leader_name && (
            <span className="text-[11px] text-[#b08d57]/70 font-medium">
              Led by <span className="text-white/30 ml-0.5">{organization.leader_name}</span>
            </span>
          )}
        </div>
      </div>
