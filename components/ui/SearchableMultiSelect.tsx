...
export default function SearchableMultiSelect({
  ...
  if (hasValue) {
    if (selectedOptions.length === 1) {
      ...
      displayLabel = selectedOptions.map(o => o.label).join(", ");
    } else {
  ...
  return (
    <div ref={rootRef} className="relative flex-1 min-w-0 w-full">
      <button
        ...
        className={`group flex w-full items-center justify-between gap-3 rounded-lg border border-white/[0.08] bg-white/[0.025] pl-3 pr-2 py-2 text-left text-[12px] transition-all duration-200 hover:border-white/16 hover:bg-white/[0.04] focus:outline-none focus-visible:border-[#b08d57]/35 focus-visible:bg-white/[0.04] disabled:cursor-not-allowed disabled:opacity-45 ${triggerClassName}`}
      >
