...
async function fetchEntityFeed(
  ...
  if (entityType === "person") {
    query = query.eq("author_profile_id", entityId);
  } else if (entityType === "business") {
    query = query.eq("author_business_id", entityId);
  } else if (entityType === "organization") {
    query = query.eq("author_organization_id", entityId);
  }
...
export default function EntityFeed({
  ...
  const load = useCallback(async (
    ...
    try {
      const result = await fetchEntityFeed(entityType, entityId, uid, currentOffset);
      setPosts((prev) => append ? [...prev, ...result.posts] : result.posts);
      setHasMore(result.hasMore);
      setOffset(currentOffset + result.posts.length);
    } catch (err: any) {
      console.error("EntityFeed fetch error:", err);
      ...
      <div className="wac-card py-16 px-6 flex flex-col items-center justify-center text-center space-y-6">
        ...
        <div className="space-y-1.5 max-w-[320px]">
          ...
          <p className="text-[13px] text-white/40 leading-relaxed text-balance">
            {emptyStateDesc || "This feed is empty."}
          </p>
        </div>
      </div>
    );
